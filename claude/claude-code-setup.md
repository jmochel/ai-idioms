# Claude Code [`CC`] Set up 


## Install latest LTS Node.js 

Avoids compatibility issues with certain MCP servers.
...
## Create ZSH or BASH alias to get to projects folder quickly
...
Cursor Example: Install the Cursor command integration, then press `Cmd+Escape` to launch `CC` within your editor. This connects your file selections with Claude's context, allowing you to work on specific code segments

## Get clear on `CC` modes

Hint: Start new features in plan mode, switch to auto-accept for implementation, and then return to interactive mode for final reviews
...
*   
## Start the CLI in the correct folder

`CC`’s effectiveness depends on starting from the correct directory. Always navigate to your project root before initializing a session. `CC` now has access to your complete project structure and can understand the full context of your codebase.

## Initial Auth and Login

Start in the correct folder.  And run `claude`
You'll be prompted to authenticate. The system will guide you through a browser-based authentication process.

## GROK the basic commands
...
* `claude -p "explain this function"` # Print mode (SDK) - execute and exit  
* `cat logs.txt | claude -p "explain"` # Process piped content
* `claude -c` # Continue most recent conversation  
* `claude -c -p "Check for type errors"`   # Continue via SDK  
...
## Master the User Global CLAUDE.md File

Copy the `ai-prompts/claude/user-global-claude.md` to `~/.claude/CLAUDE.md`

## Run Your First /init Command (The Init Command Is Not Optional)

Execute `/init` in your project directory.

This command analyzes your entire project structure and creates a CLAUDE.md file in the CWD containing:
...
### Master the Project's CLAUDE.md File

The CLAUDE.md file is Claude’s persistent context for the project 

CLAUDE.md should probably includes:
...
**Note**: Update your CLAUDE.md regularly by running /init after major changes.

### Master the User Global 
...
`CLAUDE.md` files become part of Claude’s prompts, so they should be refined like any frequently used prompt. A common mistake is adding extensive content without iterating on its effectiveness. Take time to experiment and determine what produces the best instruction following from the model.
...
Example: command that you could use to automatically pull and fix a Github issue
...
points for UI development, and visual charts for analysis and debugging. If you are not adding visuals to context, it can still be helpful to be clear with Claude about how important it is for the result to be visually appealing
