...
<each-repo>/
  .cursor/
    rules/                    # usually symlinks to ai-idioms/cursor/project/rules/*
    rules.local/              # (optional) project-specific overrides/additions
    mcp.json                  # generated by merge script (base + overlay)
    project.mcp.overlay.json  # (checked-in) partial overrides
```
